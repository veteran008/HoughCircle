...
public:
	...
	void accumulate(cv::Mat& input_img){
		...
		for (int j=0; j<nl; ++j) {
			...
			for (int i=0; i<nc; ++i) {  		
				if ( data[i]==255  ) // consider only white points (contour)
				{
					...
					if (angleindex == intervals) angleindex=intervals-1;// -90癮ngle and +90?has same effect
		...
		for (int r=0; r<R; ++r) {  // rotation
			...
			int reff = r-r0;			//because phi is (-pi ~ pi)
			...
			for (std::vector<std::vector<Vec2i>>::size_type ii = 0; ii < Rtable.size(); ++ii){
				for (std::vector<Vec2i>::size_type jj= 0; jj < Rtable[ii].size(); ++jj){
					int iimod = (ii+reff) % intervals;		//??????????		//shoule correspond to theta
	...
	void bestCandidate(){
		...
		Vec2i referenceP = Vec2i(id_max[0]*rangeXY/*+(rangeXY+1)/2*/, id_max[1]*rangeXY/*+(rangeXY+1)/2*/);		//???why plus (rangeXY+1)/2
...
private:
		...
		for (int j=0; j<nl; ++j) {
			...
			for (int i=0; i<nc; ++i) {  		
				if ( data[i]==Vec3b(255,255,255)  )
				{
					...
					rpt.phi = ((a > 0) ? a-pi/2 : a+pi/2);	//-pi ~pi convert to -pi/2 ~ pi/2
	...
	void readRtable(){
		...
		for (vector<Rpoint>::size_type t = 0; t < pts.size(); ++t){
			int angleindex = (int)((pts[t].phi+pi/2)/range);	//0 ~ 15,should be * (inv_range)
